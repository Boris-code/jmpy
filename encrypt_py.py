...
def start_encrypt(input_file_path, output_file_path, ignore_files):
    ...
    if not output_file_path: # 无输出路径
        if os.path.isdir(input_file_path): # 如果输入路径是文件夹 则输出路径为input_file_path/build
            output_file_path = get_abs_path('build', parent_path = input_file_path)
        else:
            output_file_path = get_abs_path(get_abs_path(os.path.basename(input_file_path), 'build'), os.path.dirname(input_file_path)) # 如果输入是文件 则输出文件路径为当前文件目录下的build路径
    ...
    # if not output_file_path.endswith('.py'): # 以原工程文件夹命名
    #     output_file_path = get_abs_path(os.path.basename(input_file_path), output_file_path)
